 ...
 */
static uint8_t nRF24_send_cmd(const nrf_cmd cmd);
...
{
    nRF24_send_cmd(NRF_CMD_REUSE_TX_PL);
}
...
{
    nRF24_send_cmd(NRF_CMD_FLUSH_RX);
}
...
{
    nRF24_send_cmd(NRF_CMD_FLUSH_TX);
}
 ...
 * @note Used in RX mode.
 */
void nRF24_cmd_write_ack_payload(const nrf_pipe pipe,
...
{
    return nRF24_send_cmd(NRF_CMD_NOP);
}
